...
#          2) Login (get OTP)
#          3) Verify OTP
#          4) Get user detail (authenticated)
#          5) Update account
#          6) Logout
#         """
#
...
#
#         # After successful OTP verification, the user should be logged in
#         # 4) Get user detail (authenticated endpoint)
...
#         self.assertEqual(response.data['address'], None)  # since we didn't set it
#
...
#
#         # Confirm that protected endpoint is no longer accessible without logging in again
#         response = self.client.get(self.me_url, format='json')
#         # Because you're using Session Authentication + DRF, you'll typically get 403 or 401
#         # when hitting a protected endpoint after logging out. Let's check for 403 (forbidden).
...
#         )
#         self.account = Account.objects.create(user=self.user, phone_number='1234567890')
#
#     def test_signup(self):
#         response = self.client.post('/api/account/signup/', {
#             'username': 'newuser',
...
#         self.account.otp = '123456'
#         self.account.otp_expiry = now() + timedelta(minutes=3)
#         self.account.save()
#         response = self.client.post('/api/account/verify-otp/', {
#             'username': 'testuser',
#             'otp': '123456'
#         })
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
...
#         response = self.client.post('/api/account/logout/')
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
#         self.assertIn('message', response.data)
#
